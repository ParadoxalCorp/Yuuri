class Command {
    ...
    parseCommand(message, client) {
        return new Promise(async(resolve, reject) => {
            ...
            return resolve(client.commands.get(args[1]));
        });
    }
    ...
    hasPermissions(message, client, permissions, channel = message.channel) {
        ...
        function hasPerm(perm) {
            ...
            if (!clientMember.permission.has(perm) && (!this.hasChannelOverwrite(channel, clientMember, perm) ||
                    !this.hasChannelOverwrite(channel, clientMember, perm).has(perm))) {
                return false;
            }
            return true;
        }
    ...
    hasChannelOverwrite(channel, member, permission) {
        const channelOverwrites = (new(require('./Collection'))(channel.permissionOverwrites().values()))
            .filter(co => typeof co.json[permission] !== "undefined" &&
                (co.id === member.id || member.roles.includes(id)));
        if (!channelOverwrites.size) {
            return false;
        } else if (channelOverwrites.filter(co => co.type === "user").size) {
            return channelOverwrites.filter(co => co.type === "user").first();
        }
        return channelOverwrites.sort((a, b) => channel.guild.roles.get(b.id).position - channel.guild.roles.get(a.id).position).first();
    }
}
